...
def install(v: int, venv:str, cuda: bool):
    ...
    if  sys.version_info < (3, 10, 12):
        print("python 3.10.12 or higher")
...
@click.command()
@click.option("--r", default=21, help="release version of ml-agents")
def editors(r: int):
    """
    get the supported Unity Editor versions for the given ml-agents release.
    """
    os_type = OSType(pf.system())
    unity_installer = UnityInstaller(os_type)
    versions = unity_installer.fetch_versions()
    print(versions)

def main():
    cli.add_command(install)
    cli.add_command(editors)
    cli()